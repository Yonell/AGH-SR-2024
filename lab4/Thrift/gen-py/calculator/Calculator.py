...
class Client(Iface):
    ...
    def avg(self, val):
        ...
        """
        self.send_avg(val)
        return self.recv_avg()

    def send_avg(self, val):
        self._oprot.writeMessageBegin('avg', TMessageType.CALL, self._seqid)
        args = avg_args()
        args.val = val
        args.write(self._oprot)
        self._oprot.writeMessageEnd()
    ...
    def recv_avg(self):
        ...
        if result.success is not None:
            return result.success
        raise TApplicationException(TApplicationException.MISSING_RESULT, "avg failed: unknown result")
...
class Processor(Iface, TProcessor):
    def __init__(self, handler):
        ...
        self._processMap["avg"] = Processor.process_avg
    ...
    def process_avg(self, seqid, iprot, oprot):
        args = avg_args()
        args.read(iprot)
        iprot.readMessageEnd()
        result = avg_result()
        try:
            result.success = self._handler.avg(args.val)
            msg_type = TMessageType.REPLY
        except TTransport.TTransportException:
            raise
        except TApplicationException as ex:
            ...
            msg_type = TMessageType.EXCEPTION
            result = TApplicationException(TApplicationException.INTERNAL_ERROR, 'Internal error')
        oprot.writeMessageBegin("avg", msg_type, seqid)
        result.write(oprot)
        oprot.writeMessageEnd()
        oprot.trans.flush()
...
class avg_args(object):
    ...
    def write(self, oprot):
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.writeStructBegin('avg_args')
    ...
    def __ne__(self, other):
        return not (self == other)
all_structs.append(avg_args)
avg_args.thrift_spec = (
    None,  # 0
    (1, TType.LIST, 'val', (TType.DOUBLE, None, False), None, ),  # 1
)


class avg_result(object):
    """
    Attributes:
     - success
    ...
    def write(self, oprot):
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.writeStructBegin('avg_result')
        if self.success is not None:
    ...
    def __ne__(self, other):
        return not (self == other)
all_structs.append(avg_result)
avg_result.thrift_spec = (
