...
class InvalidOperation(TException):
    ...
    def validate(self):
        if self.whatOp is None:
            raise TProtocolException(message='Required field whatOp is unset!')
        return
...
class InvalidArguments(TException):
    ...
    def __init__(self, argNo=None, reason=None,):
        super(InvalidArguments, self).__setattr__('argNo', argNo)
        super(InvalidArguments, self).__setattr__('reason', reason)
    ...
    def __hash__(self):
        return hash(self.__class__) ^ hash((self.argNo, self.reason, ))
